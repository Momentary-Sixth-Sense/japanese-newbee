...
export const KatakanaQuiz: React.FC<KatakanaQuizProps> = ({ onGoHome, selectedCharacters }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (correct) {
      ...
      const isAlreadyWrong = wrongQuestions.some(q => q.katakana === currentKatakana.katakana);
      if (!isAlreadyWrong) {
        setWrongQuestions([...wrongQuestions, currentKatakana]);
      }
    }
    ...
    setTimeout(() => {
      ...
      if (currentQuestion + 1 >= shuffledQuestions.length) {
        // 라운드 완료
        if (wrongQuestions.length > 0 || (!correct && !wrongQuestions.some(q => q.katakana === currentKatakana.katakana))) {
          // 틀린 문제가 있으면 재시도 라운드
          const finalWrongQuestions = correct ? wrongQuestions : 
            wrongQuestions.some(q => q.katakana === currentKatakana.katakana) ? wrongQuestions : 
            [...wrongQuestions, currentKatakana];
